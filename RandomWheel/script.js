let prizes = [
  { name: "⬜ Blind box", count: 147, color: "#f7d794" },
  { name: "🟨 Đèn ngủ", count: 69, color: "#ff7675" },
  { name: "🟥 Combo Bút + Tẩy", count: 100, color: "#74b9ff" },
  { name: "🟦 Móc khóa Độc Quyền", count: 88, color: "#ffffffff" },
];
...
// THÊM CÁC PHẦN TỬ THÔNG BÁO MỚI
...
// HÀM HIỂN THỊ VÀ ẨN THÔNG BÁO SAU 3 GIÂY
function showAlert(message) {
  alertMessageEl.textContent = message;
  customAlertEl.classList.add("show");

  // Tự động ẩn sau 3000ms (3 giây)
  setTimeout(() => {
...
wheel.addEventListener("transitionend", () => {
  ...
  if (currentResult) {
    ...
    showAlert(`🎉 Bạn trúng: ${currentResult.name}!`);
  ...
  // ... (giữ nguyên phần reset góc) ...
  const available = prizes.filter((p) => p.count > 0);
  const step = 360 / available.length;
  // Cần tìm lại index trong mảng available hiện tại (sau khi quay)
